...
test('be able to put falling gem', () => {
    const astate = createAnimationState();
    astate.put(new FallingGem(0, 0, gemColors.red));
    expect(astate.get(0, 0)).toBeTruthy();
});

test('falling gem must always fall', () => {
    let gem = new FallingGem(0, 0, gemColors.red);
    let y1 = gem.y;
    gem.animate();
    let y2 = gem.y;
    gem.animate();
...
test('swapping gem must always move (visually)', () => {
    let gem = new SwappingGem({x:1, y:1}, {x:2, y:2}, gemColors.red, 1000);
    let x1 = gem.offset.x;
    gem.animate();
    let x2 = gem.offset.x;
    gem.animate();
...
test('swapping gem must always arrive in time', () => {
    let gem = new SwappingGem({x:1, y:1}, {x:2.4, y:2.4}, gemColors.red, 4);
    let x1 = gem.offset.x;
    let y1 = gem.offset.y;
    gem.animate();
    gem.animate();
...
test('state must find gems in requested cell or above (with offset)', () => {
    const astate = createAnimationState();
    astate.put(new FallingGem(1, 1, gemColors.red, 0.1));
    expect(astate.get(1, 1).cell.color).toBe(gemColors.red);
    expect(astate.get(2, 1).cell).toBe(null);
    astate.animate();
    expect(astate.get(1, 2).cell.color).toBe(gemColors.red);
